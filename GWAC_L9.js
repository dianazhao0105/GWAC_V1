/***************************************
 * GWAC core module for Landsat 9
 * 
...
/***************************************
 * Spectral constants for Landsat 9
 ***************************************/

var bandKeys = ee.List(['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7']);
var bandWaves = ee.List([443, 482, 561, 654, 865, 1608, 2201]);
...
var tau = ee.List([
  0.236281717, 0.16826313, 0.09024525, 0.047937318,
  0.015505448, 0.001282477, 0.000366299
]);
var tau_all = ee.Dictionary.fromLists(bandKeys, tau);

// Ozone
var k_oz = ee.List([
  0.00284878, 0.019689206, 0.103966618, 0.06201337,
  0.002221071, 0.00000000, 0.00000000
]);
var ozone_all = ee.Dictionary.fromLists(bandKeys, k_oz);

// F0
var F0 = ee.List([
  1890.303677, 2005.275674, 1821.884875, 1550.516064,
  951.2051609, 247.8779866, 85.50309042
]);
 ...
 *
 * Landsat 9 angles are stored as degree × 100.
 *
 * @param {ee.Image} saa
 * @param {ee.Image} vaa
 * @return {ee.Image}
...
/**
 * Convert DN to TOA spectral radiance for Landsat 9 C2 L1.
 *
 * @param {ee.Image} img
 * @param {ee.List} bands
 * @return {ee.Image}
...
function getAngstrom(img) {
  ...
  var n5 = img.select('B5').divide(f0_5).max(eps);
  var n6 = img.select('B6').divide(f0_6).max(eps);
  var n7 = img.select('B7').divide(f0_7).max(eps);
...
exports.getAmstrong = getAngstrom;
...
/**
 * Run the GWAC workflow on Landsat 9 input.
 *
